...
class PHPParser:
    ...
    def _extract_services(self) -> List[str]:
        """Extrae servicios del código PHP"""
        services = set()
        ...
        for match in matches:
            ...
            # Si contiene Services o es una interfaz conocida, es un servicio
            if (
                lib and 
                ('Services' in lib or
                ...
                services.add(lib)
                self.logger.debug(f"Servicio encontrado: {lib}")
    ...
    def _extract_services(self) -> List[str]:
        """Extrae servicios inyectados"""
        services = []
        pattern = r'@inject\s*\(["\']([^"\']+)["\']'
        
        matches = re.finditer(pattern, self.content)
        services.extend(match.group(1) for match in matches)
    ...
    def get_git_changes(self, num_commits=1, since_date=None):
        """Obtiene los cambios git del archivo"""
        try:
            if since_date:
                # Obtener commits desde una fecha específica
                from datetime import datetime
                try:
                    # Validar formato de fecha
                    datetime.strptime(since_date, '%Y-%m-%d')
                except ValueError:
                    self.logger.error(f"Formato de fecha inválido: {since_date}. Use YYYY-MM-DD")
                    return []
                    
                # Usar la función get_file_history con parámetro de fecha
                from documentador.git.git_utils import get_file_history_since
                commits = get_file_history_since(self.file_path, since_date)
            else:
                # Usar las funciones existentes de git_utils con número de commits
                from documentador.git.git_utils import get_file_history
                commits = get_file_history(self.file_path, num_commits)
            
            self.logger.debug(f"Commits encontrados: {len(commits)}")
            
            changes = []
            for commit in commits:
                # Obtener el diff para este commit
                from documentador.git.git_utils import get_file_diff
                diff = get_file_diff(self.file_path, commit['hash'])
                
                if diff:
                    # Añadir el diff al commit
                    commit['diff'] = diff
                    
                    # Analizar los elementos afectados (opcional)
                    commit['affected'] = self._analyze_git_diff(diff)
            ...
            self.logger.error(f"Error obteniendo cambios git: {str(e)}")
            return []
            
    def _analyze_git_diff(self, diff):
        """Analiza un diff para identificar elementos afectados"""
        affected = {
            'functions': set(),
            'variables': set(),
            'tables': set()
        ...
        # Buscar funciones en el diff
        func_pattern = r'[\+\-]\s*(?:function|public function|private function|protected function)\s+(\w+)'
        for match in re.finditer(func_pattern, diff):
            affected['functions'].add(match.group(1))
        
        # Buscar variables en el diff
        var_pattern = r'[\+\-]\s*\$(\w+)\s*='
        for match in re.finditer(var_pattern, diff):
            affected['variables'].add(match.group(1))
        
        # Buscar tablas SQL en el diff
        table_patterns = [
            r'[\+\-].*\bFROM\s+(\w+)',
            r'[\+\-].*\bJOIN\s+(\w+)',
            r'[\+\-].*\bUPDATE\s+(\w+)',
        ...
        for pattern in table_patterns:
            for match in re.finditer(pattern, diff, re.IGNORECASE):
                affected['tables'].add(match.group(1))
